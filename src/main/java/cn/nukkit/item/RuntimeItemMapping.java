...
public class RuntimeItemMapping {
    ...
    public Item getItemByNamespaceId(@Nonnull String namespaceId, int amount) {
        ...
        if (RuntimeItems.hasData(legacyFullId)) {
            return Item.get(RuntimeItems.getId(legacyFullId), RuntimeItems.getData(legacyFullId), amount, EmptyArrays.EMPTY_BYTES);
        } else {
            Item item = Item.get(RuntimeItems.getId(legacyFullId), 0, amount, EmptyArrays.EMPTY_BYTES);
            return item;
        }
    }
