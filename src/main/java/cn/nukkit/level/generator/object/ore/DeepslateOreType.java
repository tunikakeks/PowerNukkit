...
public class DeepslateOreType {

    @PowerNukkitOnly
    @Since("1.4.0.0-PN")
    public static final DeepslateOreType[] EMPTY_ARRAY = new DeepslateOreType[0];
    ...
    @PowerNukkitOnly
    @Since("1.3.0.0-PN") public final int deepslateBlockId;

    @PowerNukkitOnly
    @Since("1.3.0.0-PN") public final int deepslateBlockData;
    ...
    public DeepslateOreType(Block material, Block deepslateMaterial, int clusterCount, int clusterSize, int minHeight, int maxHeight) {
        this.fullId = material.getFullId();
        this.blockId = material.getId();
        this.blockData = material.getDamage();
        this.deepslateBlockId = deepslateMaterial.getId();
        this.deepslateBlockData = deepslateMaterial.getDamage();
                        ...
                        if (xVal * xVal + yVal * yVal < 1.0D) {
                            for (int zSeg = minZ; zSeg <= maxZ; ++zSeg) {
                                ...
                                if (xVal * xVal + yVal * yVal + zVal * zVal < 1.0D) {
                                    int id = level.getBlockIdAt(xSeg, ySeg, zSeg);
                                    if (id == BlockID.STONE) {
                                        level.setBlockAt(xSeg, ySeg, zSeg, blockId, blockData);
                                    } else if(id == BlockID.DEEPSLATE || id == BlockID.TUFF) {
                                        level.setBlockAt(xSeg, ySeg, zSeg, deepslateBlockId, deepslateBlockData);
                                    }
                                }
                            }
                        }
